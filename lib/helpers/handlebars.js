...
/**
 * Converts swagger type name to JS type.
 */
Handlebars.registerHelper('jsType', (str) => {
  if (str === 'integer') {
    return 'Number';
  }
  return _.capitalize(str);
 ...
 */
Handlebars.registerHelper('or', (p1, p2) => {
  return p1 || p2;
});
...
Handlebars.registerHelper('contains', (array, item) => {
  return array && array.length && array.includes(item);
});

/**
 * Converts swagger schema into Joi schema definition for object type.
 */
Handlebars.registerPartial('joiObject',
`Joi.object().keys({
  {{#each schema.properties}}
  {{{quote @key}}}: {{> joiDefinition schema=. required=(contains ../schema/required @key)}},
  {{/each}}
})`);

/**
 * Converts swagger schema into Joi schema definition for simple types.
 */
Handlebars.registerPartial('joiSimpleType',
  'Joi.{{#equal schema.type "integer"}}number{{else}}{{../schema/type}}{{/equal}}()');

/**
 * Converts swagger schema into Joi schema definition.
 */
Handlebars.registerPartial('joiDefinition',
  '{{#equal (or schema.type "object") "object"}}{{> joiObject schema=../schema}}' +
  '{{else}}{{> joiSimpleType schema=../schema}}{{/equal}}' +
  '{{#if required}}.required(){{/if}}');

/**
 * Converts swagger schema into example JSON value for object type.
 */
Handlebars.registerPartial('exampleObject',
`{
    {{#each schema.properties}}
    {{{quote @key}}}: {{> exampleItem schema=.}},
    {{/each}}
  }`);

/**
 * Converts swagger schema into example JSON value for simple types.
 */
Handlebars.registerPartial('exampleSimpleType',
  '{{#equal schema.type "integer"}}1{{else}}{{#equal ../schema/type "number"}}1.0{{else}}""{{/equal}}{{/equal}}');

/**
 * Converts swagger schema into example JSON value
 */
Handlebars.registerPartial('exampleItem',
  '{{#equal (or schema.type "object") "object"}}{{> exampleObject schema=../schema}}{{else}}{{> exampleSimpleType schema=../schema}}{{/equal}}');
